    ...
    <>
      {!onlyCustomizeButton && (
        <>
          ...
          <div className="min-h-screen bg-gradient-to-br from-background via-muted/20 to-accent/10 dark:from-background dark:via-muted/10 dark:to-accent/5 transition-colors duration-500">
            {/* HEADER */}
            <header className="sticky top-0 z-50 bg-card/70 dark:bg-card/70 backdrop-blur-md border-b border-border shadow-sm">
...
const AnimatedButton: React.FC<AnimatedButtonProps> = ({ icon, label, onClick }) => (
  ...
  >
    <Button
      variant="outline"
      onClick={onClick}
      className="relative overflow-hidden bg-card dark:bg-card backdrop-blur-md 
                 border border-border hover:border-primary hover:text-primary hover:bg-accent
                 transition-all duration-300 flex items-center gap-1 sm:gap-2 text-base
