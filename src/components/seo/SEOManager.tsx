...
const SEOManager = ({ 
  ...
  useEffect(() => {
    ...
    // --- Build Title (Format: sendername-wishes-eventname-receivername) ---
    let finalTitle = title 
      || `${senderName !== "Someone" ? senderName : 'Someone'} wishes ${eventDisplay}${receiverName !== "You" ? ` to ${receiverName}` : ''}`;

    // --- Build Description (Dynamic based on event name and first text message) ---
    let finalDescription = description 
      || (firstText 
          ? `${eventDisplay} greeting: ${firstText.length > 140 ? `${firstText.substring(0, 137)}...` : firstText}`
          : `${senderName !== "Someone" ? senderName : 'Someone'} created a beautiful ${eventDisplay} greeting${greetingData?.media?.length ? ` with ${greetingData.media.length} ${greetingData.media.length === 1 ? 'image' : 'images'}` : ''} and heartfelt wishes.`);
