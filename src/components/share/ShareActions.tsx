...
const ShareActions = ({ greetingData, selectedEvent, onlyShareButton, dialogOpen, onDialogChange }: ShareActionsProps) => {
  ...
  const [currentGreetingData, setCurrentGreetingData] = useState(greetingData);
  ...
  // Listen for custom event from ShareNameInput
  useEffect(() => {
    const handleOpenShareDialog = (event: CustomEvent) => {
      const { greetingData: updatedData } = event.detail;
      if (updatedData) {
        setCurrentGreetingData(updatedData);
        setShareDialogOpen(true);
      }
    };

    window.addEventListener('openShareDialog', handleOpenShareDialog as EventListener);
    return () => {
      window.removeEventListener('openShareDialog', handleOpenShareDialog as EventListener);
    };
  }, [setShareDialogOpen]);

  // Update current greeting data when props change
  useEffect(() => {
    setCurrentGreetingData(greetingData);
  }, [greetingData]);
  ...
  const generateShareableURL = () => generateFirebaseSlugURL(currentGreetingData || {});
      ...
      <Dialog open={shareDialogOpen} onOpenChange={setShareDialogOpen}>
        <DialogContent className="max-w-md rounded-xl p-6">
          <DialogHeader>
            ...
            {/* 👇 "Sharing as" line */}
            {currentGreetingData.senderName && (
              <small className="mt-2 text-center text-sm text-muted-foreground">
                Sharing as: {currentGreetingData.senderName}
              </small>
            )}
          </DialogHeader>
