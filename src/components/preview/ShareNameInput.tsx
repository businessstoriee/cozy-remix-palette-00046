...
const ShareNameInput: React.FC<ShareNameInputProps> = ({ greetingData, greetingSlug }) => {
  ...
  const handleShare = async () => {
    ...
    if (!greetingSlug) {
      toast({
        title: "Error",
        description: "Cannot create greeting - no original greeting found.",
        variant: "destructive"
      });
      return;
    }
    ...
    try {
      ...
      if (newSlug) {
        // Update greeting data with new sender name for share dialog
        ...
        const shareableURL = `${window.location.origin}/${newSlug}`;
        await navigator.clipboard.writeText(shareableURL);
        
        toast({
          title: "Link Copied! 🎉",
          description: "Your personalized greeting link has been copied.",
        });
        
        setName('');
        
        // Trigger share dialog (ShareActions will handle it)
        const event = new CustomEvent('openShareDialog', { 
          detail: { greetingData: updatedGreetingData, slug: newSlug } 
        });
    ...
    >
      <div className="max-w-3xl my-5 z-10 mx-auto">
        ...
        >
          ...
          <div className="relative flex items-center gap-2 mx-4 sm:gap-3 p-3 bg-card dark:bg-card backdrop-blur-xl rounded-2xl border border-primary/30 dark:border-primary/30 hover:border-primary shadow-2xl">
