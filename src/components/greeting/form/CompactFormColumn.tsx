...
const CompactFormColumn: React.FC<CompactFormColumnProps> = ({
  ...
  const getTabBadgeCount = (tab: string) => {
    switch (tab) {
      case 'content':
        return formData.texts?.length || 0;
      case 'media':
        return formData.media?.length || 0;
      case 'emojis':
        return formData.emojis?.length || 0;
      default:
        return 0;
    }
  }; // ← MISSING CLOSING BRACE WAS HERE

  // Optional: Combined count for a summary badge
    ...
    <Card className="w-full border-2 border-primary/20 shadow-xl bg-gradient-to-br from-background to-secondary/10">
      <CardContent className="space-y-6 mt-6">
        <Tabs defaultValue="basics" className="w-full">
          <TabsList className="border-t-purple-500 border-t-2 sticky top-0 z-30 shadow-lg grid w-full grid-cols-4 h-auto p-1 gap-1 bg-background/85 backdrop-blur-sm rounded-lg">
            {/* Basics Tab */}
            <TabsTrigger 
              value="basics" 
            ...
            <TabsContent value="design" className="space-y-4 mt-0 animate-fade-in">
              <div className="space-y-6">
                <CustomizationForm
                  ...
                  senderNameStyle={formData.senderNameStyle}
                  receiverNameStyle={formData.receiverNameStyle}
                  flipCard={formData.flipCard}
                  interactiveReveal={formData.interactiveReveal}
                  ...
                  onSenderNameStyleChange={onSenderNameStyleChange}
                  onReceiverNameStyleChange={onReceiverNameStyleChange}
                  onFlipCardChange={onFlipCardChange}
                  onInteractiveRevealChange={onInteractiveRevealChange}
