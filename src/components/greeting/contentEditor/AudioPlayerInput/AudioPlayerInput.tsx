...
const extractInstagramReelId = (urlStr: string): string | null => {
  if (!urlStr) return null;
  try {
    const url = new URL(urlStr);
    // Instagram reel URLs: https://www.instagram.com/reel/REEL_ID/
    if (url.hostname.includes('instagram.com')) {
      const pathMatch = url.pathname.match(/\/reel\/([A-Za-z0-9_-]+)/);
...
const extractYouTubeVideoId = (urlStr: string): string | null => {
  ...
  useEffect(() => {
    ...
    if (isInstagram(value)) {
      const reelId = extractInstagramReelId(value);
      if (!reelId) {
        setStatusMsg("Invalid Instagram Reel URL");
        setIsLoading(false);
        return;
      }

      // Instagram reels require embedding the full page
      const embedUrl = `https://www.instagram.com/reel/${reelId}/embed`;
      setStatusMsg("Instagram Reels don't support audio playback directly");
      toast({
        title: "Instagram Note",
        description: "Instagram reels can be viewed but audio playback may be limited",
      });
      setIsLoading(false);
      return;
    }
