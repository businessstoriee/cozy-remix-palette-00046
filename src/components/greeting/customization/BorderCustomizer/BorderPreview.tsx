...
const BorderPreview: React.FC<PreviewProps> = ({ settings, width = '100%', height = 120, compact = false }) => {
  ...
  // travel computed positions map
  const [travelPositions, setTravelPositions] = React.useState<Record<string, { left: number; top: number }>>({});
  ...
  // RAF loop for travel elements (continuous movement along border in specified direction)
  React.useEffect(() => {
    let rafId = 0;
    let start = performance.now();

    const travelElements: BorderElement[] = (settings.decorativeElements || []).filter(el => el.animation === 'travel');
    ...
    const update = (now: number) => {
      ...
      travelElements.forEach(el => {
        ...
        // If traveling on a specific border side only
        if (borderSide !== 'all') {
          const inset = borderWidth / 2;
          const innerW = Math.max(0, containerSize.width - inset * 2);
          const innerH = Math.max(0, containerSize.height - inset * 2);
          ...
          // Travel around entire perimeter
          let percent = 0;
          switch (direction) {
            case 'top-down':
      ...
      setTravelPositions(prev => ({ ...prev, ...newPos }));
      rafId = requestAnimationFrame(update);
    };
      ...
      >
        ...
        {settings.decorativeElements?.map((el: BorderElement) => {
          ...
          const pos = isRevolve
            ...
            : isTravel
            ? (travelPositions[el.id] ?? computePerimeterPos(el.position ?? 0, elSize))
            : (staticPositions[el.id] || computePerimeterPos(el.position ?? 0, elSize));

          // non-revolve and non-travel animations (rotate/float/etc)
          let animationCss = '';
          if (!isRevolve && !isTravel) {
            switch (el.animation) {
              case 'rotate-cw':
              ...
              case 'pulse':
                ...
                break;
               case 'spin':
                animationCss = `spin ${el.rotateSpeed || 4}s linear infinite`;
                break;
              case 'spin-ccw':
                animationCss = `spinCCW ${el.rotateSpeed || 4}s linear infinite`;
                break;
              default:
                animationCss = '';
            }
